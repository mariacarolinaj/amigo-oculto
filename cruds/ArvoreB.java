...
public class ArvoreB {
    ...
    private boolean inserir1(long pagina) throws IOException {
        ...
        int i = 0;
        while (i < pa.n
                && (chave1Extra > pa.chaves1[i] || (chave1Extra == pa.chaves1[i] && chave2Extra > pa.chaves2[i]))) {
            i++;
        }
        ...
        // Novo registro deve ficar na página da direita
            ...
            int j = 0;
            for (j = maxElementos - meio; j > 0 && (chave1Extra < np.chaves1[j - 1]
                    || (chave1Extra == np.chaves1[j - 1] && chave2Extra < np.chaves2[j - 1])); j--) {
